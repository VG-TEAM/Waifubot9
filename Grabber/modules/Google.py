...
class STRINGS:
    ...
    RESULT = """
🔤 <b>Name:</b> {name}
🗒️ <b>Descriptor:</b> <i>{descriptor}</i>
...
async def on_google_lens_search(client: Client, message: Message) -> None:
    ...
    descriptor = response_json["descriptor"]
    ...
    text = STRINGS.RESULT.format(
        name=f"<code>{name}</code>" if name else "<i>Name not found</i>",
        descriptor=f"<i>{descriptor}</i>" if descriptor else "<i>Not available</i>",
